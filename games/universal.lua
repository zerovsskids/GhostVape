...
local function canClick()
	...
	return (not GhostVape.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function serverHop(pointer, filter)
	visited = shared.GhostVapeserverhoplist and shared.GhostVapeserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('GhostVape', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('GhostVape', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('GhostVape', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('GhostVape', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

GhostVape:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.GhostVapeserverhoplist = '"..table.concat(visited, '/').."'\nshared.GhostVapeserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('GhostVape/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('GhostVape/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('GhostVape/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
GhostVape.Libraries.entity = entitylib
GhostVape.Libraries.whitelist = whitelist
GhostVape.Libraries.prediction = prediction
GhostVape.Libraries.hash = hash
GhostVape.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if GhostVape.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and GhostVape.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(GhostVape.Categories.Friends.Options['Friends color'].Hue, GhostVape.Categories.Friends.Options['Friends color'].Sat, GhostVape.Categories.Friends.Options['Friends color'].Value)
		end
	...
	GhostVape:Clean(GhostVape.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	GhostVape:Clean(GhostVape.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	GhostVape:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	GhostVape:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = GhostVape.Uninject
				GhostVape.Uninject = function()
					notif('GhostVape', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('GhostVape', plr.Name..' is using GhostVape!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				GhostVape:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				GhostVape:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('GhostVape/profiles/whitelist.json') and readfile('GhostVape/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				GhostVape:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and GhostVape.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'GhostVape ANNOUNCEMENT: '..whitelist.data.Announcement.text
				...
				pcall(function()
					writefile('GhostVape/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillGhostVape then
				GhostVape:Uninject()
				return true
			end
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = GhostVape.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
		...
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not GhostVape.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = GhostVape.gui.AbsoluteSize / 2
				...
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not GhostVape.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	SilentAim = GhostVape.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	HighJump = GhostVape.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Invisible = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	MouseTP = GhostVape.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Spider = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = GhostVape.Categories.Blatant:CreateModule({
		Name = 'SpinBot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = GhostVape.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = GhostVape.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('GhostVape/assets/new/arrowmodule.png')
	...
	Arrows = GhostVape.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = GhostVape.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = GhostVape.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = GhostVape.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if GhostVape.ThreadFix then
				...
				--movingsound.SoundId = downloadGhostVapeAsset('GhostVape/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadGhostVapeAsset('GhostVape/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = GhostVape.Modules.Fly and GhostVape.Modules.Fly.Enabled or GhostVape.Modules.LongJump and GhostVape.Modules.LongJump.Enabled or GhostVape.Modules.InfiniteFly and GhostVape.Modules.InfiniteFly.Enabled
...
run(function()
	...
	PlayerModel = GhostVape.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	Radar = GhostVape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('GhostVape/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = GhostVape.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = GhostVape:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.GhostVapesessioninfo = '"..httpService:JSONEncode(GhostVape.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.GhostVapesessioninfo then
					for i, v in httpService:JSONDecode(shared.GhostVapesessioninfo) do
						if GhostVape.Libraries.sessioninfo.Objects[i] and v.Saved then
							GhostVape.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if GhostVape.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
	
						for i, v in GhostVape.Libraries.sessioninfo.Objects do
	...
	GhostVape:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if GhostVape.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (GhostVape.gui.AbsoluteSize.X / 2)
	...
	addBlur(infoholder)
	GhostVape.Libraries.sessioninfo = {
		Objects = {},
	...
	GhostVape.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = GhostVape.gui.AbsoluteSize
	...
	Tracers = GhostVape.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = GhostVape.gui
	
	Waypoints = GhostVape.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = GhostVape.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = GhostVape.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = GhostVape.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = GhostVape.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	Disabler = GhostVape.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Rejoin = GhostVape.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = GhostVape.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.GhostVapeserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.GhostVapeserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.GhostVapeserverhopprevious)
			end
		end
	})
end)
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				...
				if GhostVape.Profile ~= Profile.Value then
					GhostVape.Profile = Profile.Value
...
run(function()
	...
	Freecam = GhostVape.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = GhostVape.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = GhostVape.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = GhostVape.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = GhostVape.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = GhostVape.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = GhostVape.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Cape = GhostVape.Legit:CreateModule({
		Name = 'Cape',
		Function = function(callback)
			if callback then
				part = Instance.new('Part')
...
run(function()
	...
	ChinaHat = GhostVape.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if GhostVape.ThreadFix then
...
run(function()
	...
	Clock = GhostVape.Legit:CreateModule({
		Name = 'Clock',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Disguise = GhostVape.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FPS = GhostVape.Legit:CreateModule({
		Name = 'FPS',
		Function = function(callback)
			if callback then
				local frames = {}
...
run(function()
	...
	Keystrokes = GhostVape.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = GhostVape.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Ping = GhostVape.Legit:CreateModule({
		Name = 'Ping',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Speedmeter = GhostVape.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = GhostVape.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then

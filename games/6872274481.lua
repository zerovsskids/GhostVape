...
local GhostVapeEvents = setmetatable({}, {
...
local targetinfo = GhostVape.Libraries.targetinfo
local sessioninfo = GhostVape.Libraries.sessioninfo
local uipallet = GhostVape.Libraries.uipallet
local tween = GhostVape.Libraries.tween
local color = GhostVape.Libraries.color
local whitelist = GhostVape.Libraries.whitelist
local prediction = GhostVape.Libraries.prediction
local getfontsize = GhostVape.Libraries.getfontsize
local getcustomasset = GhostVape.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('GhostVape/assets/new/blur.png')
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		GhostVapeEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
...
local function isFriend(plr, recolor)
	if GhostVape.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(GhostVape.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and GhostVape.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(GhostVape.Categories.Targets.ListEnabled, plr.Name) and true
end

local function notif(...) return
	GhostVape:CreateNotification(...)
end
...
run(function()
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						GhostVapeEvents.AttributeChanged:Fire(attr)
					end))
				else
	...
	GhostVape:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
end)
entitylib.start()
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('GhostVape', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				GhostVapeEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				GhostVapeEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not GhostVape.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			GhostVape:Clean(connection:Connect(function(...)
				GhostVapeEvents[event]:Fire(...)
			end))
		end)
	end

	GhostVape:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		GhostVapeEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		GhostVape:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
			...
			GhostVapeEvents[event]:Fire(data)
		end))
	end
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or GhostVape.Loaded == nil
			if GhostVape.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	GhostVape:Clean(GhostVapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	GhostVape:Clean(GhostVapeEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	GhostVape:Clean(GhostVapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until GhostVape.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	AimAssist = GhostVape.Categories.Combat:CreateModule({
		Name = 'AimAssist',
...
run(function()
	...
	AutoCharge = GhostVape.Categories.Combat:CreateModule({
	    Name = 'AutoCharge',
	    Function = function(callback)
...
run(function()
	...
	AutoClicker = GhostVape.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	GhostVape.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Reach = GhostVape.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
...
run(function()
	...
	TriggerBot = GhostVape.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Velocity = GhostVape.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiFall = GhostVape.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if GhostVape.Modules.Fly.Enabled or GhostVape.Modules.InfiniteFly.Enabled or GhostVape.Modules.LongJump.Enabled then
...
run(function()
	...
	FastBreak = GhostVape.Categories.Blatant:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Fly = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(GhostVapeEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	HitBoxes = GhostVape.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
...
run(function()
	GhostVape.Categories.Blatant:CreateModule({
		Name = 'KeepSprint',
		Function = function(callback)
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = GhostVape.Libraries.auraanims, tick()
	...
	Killaura = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir)
			LongJump:Clean(GhostVapeEvents.CatPounce.Event:Connect(function()
	...
	LongJump = GhostVape.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			frictionTable.LongJump = callback or nil
			updateVelocity()
			if callback then
				LongJump:Clean(GhostVapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(GhostVapeEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	NoFall = GhostVape.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				local tracked = 0
...
run(function()
	...
	local ProjectileAimbot = GhostVape.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = GhostVape.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Speed = GhostVape.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	BedESP = GhostVape.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Health = GhostVape.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
				...
				label.Parent = GhostVape.gui
				Health:Clean(label)
				Health:Clean(GhostVapeEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = GhostVape.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = GhostVape.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(GhostVape.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	StorageESP = GhostVape.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local AutoKitFunctions = {
		...
		cat = function()
			...
			bedwars.CatController.leap = function(...)
				GhostVapeEvents.CatPounce:Fire()
				return old(...)
			end
	...
	AutoKit = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoPlay = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(GhostVapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(GhostVapeEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoShoot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoToxic = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(GhostVapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(GhostVapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(GhostVapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = GhostVape.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = GhostVape.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	PickupRange = GhostVape.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RavenTP = GhostVape.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	Scaffold = GhostVape.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if label then
...
run(function()
	...
	ShopTierBypass = GhostVape.Categories.Utility:CreateModule({
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not GhostVape.Loaded then
			repeat task.wait() until GhostVape.Loaded
		end
		...
		elseif Mode.Value == 'Profile' then
			GhostVape.Save = function() end
			if GhostVape.Profile ~= Profile.Value then
				GhostVape:Load(true, Profile.Value)
			end
		elseif Mode.Value == 'AutoConfig' then
			local safe = {'AutoClicker', 'Reach', 'Sprint', 'HitFix', 'StaffDetector'}
			GhostVape.Save = function() end
			for i, v in GhostVape.Modules do
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and GhostVape.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	StaffDetector = GhostVape.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
	...
	task.spawn(function()
		repeat task.wait(1) until GhostVape.Loaded or GhostVape.Loaded == nil
		if GhostVape.Loaded and not StaffDetector.Enabled then
...
run(function()
	TrapDisabler = GhostVape.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	GhostVape.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoSuffocate = GhostVape.Categories.World:CreateModule({
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = GhostVape.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = GhostVape.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChestSteal = GhostVape.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = GhostVape.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = GhostVape.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBank = GhostVape.Categories.Inventory:CreateModule({
		Name = 'AutoBank',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = GhostVape.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
				...
				local lastupgrades
				AutoBuy:Clean(GhostVapeEvents.InventoryAmountChanged.Event:Connect(function()
...
run(function()
	...
	AutoConsume = GhostVape.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(GhostVapeEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(GhostVapeEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = GhostVape.gui.ScaledGui
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			GhostVape.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / GhostVape.guiscale.Scale)
		end)
		table.insert(GhostVape.Windows, window)
	...
	GhostVape.Components.HotbarList = function(optionsettings, children, api)
		if GhostVape.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('GhostVape/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / GhostVape.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					GhostVape.gui.ScaledGui.ClickGui.Visible = false
	...
	AutoHotbar = GhostVape.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(GhostVapeEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = GhostVape.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = GhostVape.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedPlates = GhostVape.Categories.Minigames:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				for _, v in collectionService:GetTagged('bed') do 
				...
				BedPlates:Clean(GhostVapeEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(GhostVapeEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('GhostVape/assets/new/blur.png'),
	...
	Breaker = GhostVape.Categories.Minigames:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
				for _ = 1, 30 do
...
run(function()
	...
	BedBreakEffect = GhostVape.Legit:CreateModule({
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(GhostVapeEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = GhostVape.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = GhostVape.Legit:CreateModule({
...
run(function()
	...
	FOV = GhostVape.Legit:CreateModule({
		Name = 'FOV',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FPSBoost = GhostVape.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = GhostVape.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = GhostVape.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = GhostVape.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = GhostVape.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = GhostVape.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SoundChanger = GhostVape.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = GhostVape.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	Viewmodel = GhostVape.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = GhostVape.Legit:CreateModule({
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(GhostVapeEvents.MatchEndEvent.Event:Connect(function()
...
run(function()
  ...
  texturepack = GhostVape.Categories.Render:CreateModule({
    Name = "TexturePack",
...
run(function()
  ...
  local packselected = {["Value"] = "OldBedwars"}
	...
	TexturePacks = GhostVape.Categories.Render:CreateModule({
		["Name"] = "TexturePacksV3",
		["Function"] = function(callback) 
			if callback then 
...
run(function()
	...
	VeloAntiHit = GhostVape.Categories.Blatant:CreateModule({
		Name = "AntiHit",
...
run(function()
    ...
    Invisibility = GhostVape.Categories.Blatant:CreateModule({
        Name = 'Invisibility',
        Function = function(callback)
            ...
            if callback then
                GhostVape:CreateNotification('Invisibility Enabled', 'You are now invisible.', 4)
                modifyHRP(true)
...
run(function()
    local InfiniteJump
    local Velocity
    InfiniteJump = GhostVape.Categories.Blatant:CreateModule({
        Name = "InfiniteJump",
        Function = function(callback)
            if callback then
...
run(function()
	...
	PartyPopperExploit = GhostVape.Categories.Blatant:CreateModule({
...
run(function()
	...
	TrainWhistleExploit = GhostVape.Categories.Blatant:CreateModule({
...
run(function()
    ...
    AdetundeExploit = GhostVape.Categories.World:CreateModule({
...
run(function()
    ...
    ClickDash = GhostVape.Categories.Utility:CreateModule({
        Name = "ClickDash",
        Function = function(callback)
...
run(function()
	...
	YuziFly = GhostVape.Categories.Blatant:CreateModule({
		Name = "YuziFly",
		Function = function(enabled)
...
run(function()
	...
	studantivoid = GhostVape.Categories.Blatant:CreateModule({
		Name = "StudAntiVoid",
		Function = function(callback)
			if callback then
